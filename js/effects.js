...
import { hexRgb, blendHex } from './bodies/Body.js';
...
export class Effects {
  ...
  draw(ctx, camera, canvas) {
    ...
    for (const f of this.flashes) {
      ...
      const a  = f.life;
      const blended = blendHex(f.colorA, f.colorB, 0.5);
      const grad = ctx.createRadialGradient(sp.x, sp.y, 0, sp.x, sp.y, sr * (2 - f.life));
      const [r,g,b] = hexRgb(blended);
  ...
  get active() {
    return this.particles.length + this.shockwaves.length +
           this.flashes.length   + this.floaters.length > 0;
  }
}
